...
# 이전에 처리된 ISBN 목록 로드
tagged_file = "yes24_test_tagged.csv"
...
def model_worker():
    ...
    while True:
        ...
        try:
            ...
            new_row.to_csv(tagged_file, mode="a", header=not os.path.exists(tagged_file), index=False, encoding="utf-8-sig")
...
# 데이터 로딩 및 큐 적재
df = pd.read_csv("yes24_steadyseller_isbn.csv")
for idx, row in df.iterrows():
    isbn = str(row.get("isbn", ""))
    if isbn in processed_isbns:
        continue
    title = row.get("Title", "무제")
    review = str(row.get("Review", ""))[:1000]
