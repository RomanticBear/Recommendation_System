...
def generate_prompt(text):
    ...
    [4단계] 위 분석을 종합해, 아래 제공된 태그 목록 중에서 책을 가장 정확히 표현하는 태그를 3~6개 선택하세요.
...
if os.path.exists(output_path):
    ...
    existing_df = pd.DataFrame(columns=["isbn", "tag"])
    processed_isbns = set()
        ...
        try:
            ...
            if len(tags) < 3:
                ...
                if retry_count + 1 < max_retries:
                    ...
                    new_row.to_csv(output_path, mode="a", header=write_header, index=False, encoding="utf-8-sig")
                    with processed_isbns_lock:
